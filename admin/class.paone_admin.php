...
if (!class_exists('PAONE_Admin')) {
    define('PAONE_WP_URL_ADMIN_PAGE', menu_page_url('PAONE_woocommerce_personalization', false));


    class Paone_Admin extends Paone_Admin_model
    {
        ...
        public function __construct()
        {
            ...
            add_action('paone_load_admin_settings', array($this, 'show_fields'));
            add_action('paone_enqueue_scripts_admin', array($this, 'enqueue_scripts'));


            //Do Actions
            do_action('paone_load_admin_settings');
            do_action('paone_enqueue_scripts_admin');
        ...
        function show_fields()
        {
            ...
            require_once PAONE_WP_PATH . '/admin/templates/form.php';
        }

        function enqueue_scripts()
        {
            wp_enqueue_style('', PAONE_WP_URL . '/assets/css/admin-style.css');
            wp_enqueue_script('jscolor', PAONE_WP_URL . '/assets/js/jscolor.js');
            wp_enqueue_style('select2-css', PAONE_WP_URL . '/assets/select2/css/select2.min.css');
            wp_enqueue_style('paone-css', PAONE_WP_URL . '/admin/css/styles.css');
            wp_enqueue_script('select2-js', PAONE_WP_URL . '/assets/select2/js/select2.min.js', array('jquery'));
            wp_enqueue_script('jquery-validate', PAONE_WP_URL . '/assets/js/jquery.validate.min.js', array('jquery'));
            wp_enqueue_script('paone-js', PAONE_WP_URL . '/admin/js/scripts.js', array('jquery'));
        }
        /**
        ...
        function tableInput($name = "", $headers, $data = null, $attributes = array(), $options = array())
        {
            ...
            $add_button = '<a href="javascript:void(0);" style="min-width:10px;display:block;" onclick="ti_add_rows_' . $id . '();return false;" id="ti_add_more_btn_' . $id . '" title="Add more"><img src="' . PAONE_WP_URL . '/assets/images/add-icon.png"  border="0" /></a>';
            $delete_button = '<a href="javascript:void(0);" style="min-width:10px;display:block;" onclick="ti_rows_cnt_' . $id . '--;$conv(this).parent().parent().remove();
							...
							<img src="' . PAONE_WP_URL . '/assets/images/delete-icon.png" border="0" /></a>';
            $table_head = '';
